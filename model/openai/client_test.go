...
func TestRequestOutputLimitOverridesDefaultAndRespectsHardLimit(t *testing.T) {
	...
	if defaultBody.MaxTokens == nil || *defaultBody.MaxTokens != hardLimit {
		t.Fatalf("default max_tokens = %v, want %d", defaultBody.MaxTokens, hardLimit)
	}
	for _, test := range []struct {
		requested int
		want      int
		...
		if body.MaxTokens == nil || *body.MaxTokens != test.want {
			t.Fatalf("max_tokens = %v, want %d", body.MaxTokens, test.want)
		}
	}
