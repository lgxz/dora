...
`Model` is the minimal interface that every model implementation must satisfy. `StreamingModel` and `ContextSize` are optional capabilities: when the Agent detects `StreamingModel` it uses the streaming method, otherwise it falls back to `Generate`; when it detects `ContextSize` and the reported value is positive it uses it as the context token budget for compaction, otherwise it falls back to `DefaultContextWindowTokens`. `ContextSize` reports the model context capacity in tokens and leaves the `Model` and `StreamingModel` contracts unchanged.
...
- Before building a request for the model, the Agent compacts long histories (a round is an assistant message together with the tool messages it triggered, and is never split in half). Whether to compact, and how many of the most recent rounds to retain, is decided dynamically from the current context occupancy: it retains the newest rounds until the prefix plus the retained rounds approaches a `budgetSafetyRatio` (0.9) fraction of the model-reported `contextWindow` tokens, bounded by `minRetainedRounds` (8) and `maxRetainedRounds` (`defaultCompactionRounds` = 32). During a run, the Agent keeps this model-visible history separately from the complete `Turn`: each tool round advances from the exact messages sent by the preceding request, so compaction remains in effect while the `Turn` preserves the unabridged history for persistence. The occupancy estimate anchors on the previous model call's real `total_tokens` (from `Response.Usage`), which therefore describes that same model-visible baseline, already includes the assistant response, and adds only estimated tokens for tool result messages produced afterwards. When usage is not reported it estimates the complete model-visible history, using about four ASCII bytes or one non-ASCII rune per token plus a small per-message framing allowance. When that history already fits within the budget it is returned unchanged. Retained historical rounds are further compressed against the same 0.9 budget by first subtracting the always-retained prefix and current round, then dividing the remainder among historical messages. Each historical message shares its allocation across content and all JSON tool inputs, including fixed framing and identifiers; JSON compaction bounds the complete encoded input rather than each string independently. If preserved identifiers and framing still make the aggregate exceed the budget, the oldest historical rounds are dropped whole. Images are dropped, while the current (last) round is retained unchanged. A non-positive budget disables truncation (images are still dropped). Truncation applies to a copy of the request messages, never rewriting the persisted Turn history.
